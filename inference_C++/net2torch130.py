...
class PointNetFeaturePropagation_0(nn.Module):
    def __init__(self, in_channel, mlp):
        super(PointNetFeaturePropagation_0, self).__init__()
...
class get_model_1(nn.Module):
    def __init__(self, num_classes, normal_channel=False, num_categories=16):
        super(get_model_1, self).__init__()
        self.num_categories = num_categories
        if normal_channel:
            additional_channel = 3
        else:
        ...
        self.fp3 = PointNetFeaturePropagation_0(in_channel=1536, mlp=[256, 256])
        self.fp2 = PointNetFeaturePropagation_1(in_channel=576, mlp=[256, 128])
        self.fp1 = PointNetFeaturePropagation_1(in_channel=134 + self.num_categories + additional_channel, mlp=[128, 128])
        self.conv1 = nn.Conv1d(128, 128, 1)
        self.bn1 = nn.BatchNorm1d(128)
        self.drop1 = nn.Dropout(0.5)
        self.conv2 = nn.Conv1d(128, num_classes, 1)
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        save_path = "../log/part_seg/pointnet2_part_seg_msg_add_data/checkpoints/best_model_1.3.0.pth"
        new_save_path = "../log/part_seg/pointnet2_part_seg_msg_add_data/checkpoints/best_model_1.3.0_new.pth"
        ...
        # ****** step1 模型保存 ******
        net_ref = get_model(2, normal_channel=False, num_categories=1)
        state_dict = torch.load(save_path, map_location=torch.device('cpu'))
        net_ref.load_state_dict(state_dict)
        net_ref.eval()
        net_ref.apply(freeze_bn)

        torch.save(net_ref.state_dict(), r"E:\code\Server223\pointNet\log\part_seg\pointnet2_part_seg_msg_add_data\checkpoints\test.pth")
        print("**********net params: ***********")
        net_params = {}
        bn_mean_params = {}
        bn_var_params = {}
        ...
        # ****** step2 替换参数 ******
        net = get_model_1(2, normal_channel=False, num_categories=1)
        ...
        pt_save_path = "./pointnet2_part_seg_msg_torchscript_model_.pt"
