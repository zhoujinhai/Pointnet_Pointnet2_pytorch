...
from data_utils.ToothPcdDataLoader import PartNormalDataset, my_collate_fn
...
def main(args):
    ...
    testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=10, collate_fn=my_collate_fn)
    ...
    classifier = MODEL.get_model(num_part, normal_channel=args.normal, num_categories=num_classes).cuda()
    ...
    try:
        checkpoint = torch.load(str(exp_dir) + '/checkpoints/min_loss_model.pth')
        start_epoch = checkpoint['epoch']
        classifier.load_state_dict(checkpoint['model_state_dict'])
        print("************")

        checkpoint1 = torch.load(str(exp_dir) + '/checkpoints/best_model.pth')
        best_acc = checkpoint1['test_acc']
        best_class_avg_iou = checkpoint1['class_avg_iou']
        best_instance_avg_iou = checkpoint1['instance_avg_iou']
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        if min_loss > train_loss:
            ...
            log_string('min loss is %f' % min_loss)
            state = {
                'epoch': epoch,
        ...
        log_string('Best inctance avg mIOU is: %.5f' % best_instance_avg_iou)
        global_epoch += 1
