...
import argparse
import os
from data_utils.ToothPcdDataLoader import SemSegDataset, my_collate_fn_sem
import torch
import datetime
import logging
...
def main(args):
    ...
    root = r"/home/heygears/jinhai_zhou/data/pcd_with_label"
    NUM_CLASSES = 2
    f_cols = 3
    ...
    #                                              pin_memory=True, drop_last=True)
    TRAIN_DATASET = SemSegDataset(root=root, npoints=args.npoint, split='train_val', normal_channel=args.normal)
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batch_size, shuffle=True, num_workers=10, drop_last=True, collate_fn=my_collate_fn_sem)
    TEST_DATASET = SemSegDataset(root=root, npoints=args.npoint, split='test', normal_channel=args.normal)
    testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=10, collate_fn=my_collate_fn_sem)
